 ...
 * (§6.3).
 *
 * Controls with a hand-built page of their own are excluded, and that is about
 * the build rather than the route: a static segment wins over its dynamic
 * sibling at request time, so `/features/otp-verification` would resolve to
 * the dedicated page regardless — but generating a param for a URL another
 * file already owns produces a second prerendered output for the same path,
 * one that no request will ever reach.
 *
 * Which slugs those are is read from the controls repository rather than
 * listed here, because the sitemap has to answer the same question and two
 * copies of the answer is one copy too many.
 *
...
export function generateStaticParams() {
  return getControls()
    .filter((control) => !hasDedicatedPage(control.slug))
