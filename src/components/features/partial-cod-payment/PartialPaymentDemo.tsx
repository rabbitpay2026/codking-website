...
import { CtaVideo, demoVideoUrl } from "@/components/sections/cta/CtaVideo";
import { SectionShell } from "@/components/sections/SectionShell";
import { BlurFade } from "@/components/ui/blur-fade";
import { Button } from "@/components/ui/button";
import { panelHoverClass } from "@/constants/theme";
import { getPartialDemoPoints } from "@/lib/content";
 ...
 * ratio so the row does not jump when the iframe arrives. Same component and
 * same proportions as the OTP page, so a merchant moving between the two meets
        ...
        <BlurFade delay={0.06}>
          <div className="flex flex-col items-start">
            ...
            <h2 className="mt-4 text-[1.6rem] leading-[1.12] font-semibold tracking-[-0.028em] text-balance text-ink sm:text-[1.9rem]">
              See partial payments in action
            </h2>

            <p className="mt-3.5 max-w-md text-[14px] leading-relaxed text-pretty text-ink/55">
              Watch how COD King collects an upfront amount at checkout and
              protects your store from orders nobody intended to accept.
            </p>

            <ul className="mt-5 space-y-2.5">
              {getPartialDemoPoints().map((point) => (
