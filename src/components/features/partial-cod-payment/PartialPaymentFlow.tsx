 ...
 * place this page raises its voice. The blueprint does the same, and the
 * reason holds: every other band here is evidence — figures, features, quotes
 * — and this is the only one that is a *mechanism*. Lifting it onto a panel
 * says "this is the part to actually read" without a word of copy claiming so.
 *
 ...
 * index, a mark, a title, a line, and the arrows carrying the direction is
 * all a flow needs.
 *
 * Three of the four beats are the buyer's and none are the merchant's, which
          ...
          */}
          <ol className="mt-8 grid gap-x-10 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 lg:grid-rows-[auto_auto_auto] lg:gap-y-0">
            {steps.map((step, index) => {
              const Icon = iconFor[step.id] ?? ShoppingCart;
          ...
          */}
          <p className="mx-auto mt-9 flex w-full max-w-3xl items-center justify-center gap-3 rounded-xl border border-brand-check/35 bg-brand-check/12 px-5 py-3.5 text-center">
            ...
            <span className="text-[13.5px] leading-snug font-semibold text-balance text-ink/80">
              <span className="font-bold text-ink/50">Result:</span>{" "}
              {getPartialFlowResult()}
            </span>
          </p>
        </div>
