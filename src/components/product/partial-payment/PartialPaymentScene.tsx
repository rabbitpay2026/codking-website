import { BadgeCheck, SlidersHorizontal, Truck } from "lucide-react";

import { PartialPaymentPanel } from "@/components/product/partial-payment/PartialPaymentPanel";
...
/**
 * The three things the merchant controls, ringing the thing the buyer sees.
 *
 * Worded as settings rather than as benefits, which is the division that makes
 * the scene worth having two halves: the panel is what the *customer* gets,
 * these are what the *merchant* decides. The same three words as benefits
 * would be the hero checklist a second time.
 */
const CONTROLS: readonly { id: string; icon: LucideIcon; label: string }[] = [
  {
    id: "rules",
    icon: SlidersHorizontal,
    label: "Flexible rules — % or fixed amount",
  },
  { id: "shipping", icon: Truck, label: "Cover shipping charges upfront" },
  { id: "buyers", icon: BadgeCheck, label: "Lock in genuine buyers only" },
];

/**
 * The hero's product scene.
 *
 * The checkout panel, and beside it the settings behind it. Two objects and
 * one relationship: what the buyer is shown on the left, what the merchant
 * chose on the right — so a visitor understands before reading a word that the
 * screen is configurable rather than fixed.
 *
 * The chips are the first thing to go as the column narrows. Below `lg` there
 * is no room for a column of annotation beside a checkout panel, and a chip
 ...
 */
export function PartialPaymentScene({ className }: WithClassName) {
  return (
    <div className={cn("relative w-full", className)}>
      ...
      <div className="flex items-center gap-4">
        <PartialPaymentPanel className="mx-auto w-full max-w-[23rem] lg:mx-0 lg:max-w-none lg:flex-1" />
