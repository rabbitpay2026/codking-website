...
export function MagicCard(props: MagicCardProps) {
  ...
  /*
   * The registry version reads the active theme from next-themes behind a
   * `mounted` flag set inside an effect, which costs a cascading render on
   * every card and exists only to avoid a hydration mismatch on its own
   * hardcoded default colours.
   *
   * This project has no theme provider, and callers pass colours from the
   * token layer — which already resolves per theme in CSS. So the lookup is
   * removed: one less client dependency, no extra render pass, and the card
   * follows the design tokens like everything else. The orb blend mode is
   * fixed to `multiply`, which is the light-surface value.
   */
