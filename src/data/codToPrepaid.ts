...
export const prepaidFeatures: readonly FeatureCapability[] = [
  ...
  },
  {
    /*
      ...
      says the thing it asked for instead: the COD King documentation sets a
      payment gateway up for this control by API key and secret, so the link
      the reminder carries is generated by the merchant's own gateway.

      An earlier version of this card named Razorpay alone, because Razorpay
      was the only provider the documentation named. The team has since
      confirmed the position the documentation left implicit — a merchant
      connects whichever gateway they prefer — so the card now leads with the
      choice and treats the names as examples of it. That order matters: a
      merchant on a gateway not listed has to reach the end of this sentence
      still certain the control is for them.

      The four named here are the same four the homepage shows, and for the
      same reason; see the `payment-gateway` records in
      `src/data/integrations.ts`.
    */
    id: "gateway",
    title: "Your preferred payment gateway",
    body: "Connect whichever gateway you already use — Razorpay, PayU, Cashfree and PhonePe among them — with its API key and secret, so the payment link is raised on your own account.",
  },
  {
    id: "simple",
    title: "Easy & simple",
